...
| Spectator system | Complete | Server + client, frame streaming, palette propagation, fast-forward/hard-sync |
...
Four-component spectator architecture:

**Server side:**
- `spectator_manager.cpp`: Manages up to 8 spectator clients. Handles handshake with protocol version check, per-peer state tracking, frame batch streaming, palette state/data propagation, heartbeat at 500ms intervals, redirect endpoint support.
- `spectator_runtime.cpp`: Archives rollback frames for spectators, manages match metadata (ID, config, player names), tracks palette epochs for sync.

**Client side:**
- `spectator_client.cpp`: Connects to host's spectator server, buffers frames with configurable thresholds (30-frame fast-forward, 180-frame hard-sync), palette state tracking for remote preview.
...
- **Top bar:** P1 name (blue) and P2 name (red) with win counts, shadow text for readability
- **Bottom bar:** PING (ms), delay frames, rollback frames with background rectangles
- Renders in 640x480 game coordinates scaled to display size
...
| 13 | MODE_PALETTE | Palette viewer |
