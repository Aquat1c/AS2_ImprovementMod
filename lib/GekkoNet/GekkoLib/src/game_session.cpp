...
void Gekko::GameSession::Init(GekkoConfig* config)
{
    ...
    LOG_GEKKO_INFO("[Gekko][GameSession] Init players=%u input=%u state=%u pred=%u limited=%d",
                   _config.num_players,
                   _config.input_size,
                   _config.state_size,
                   _config.input_prediction_window,
                   _config.limited_saving ? 1 : 0);
}
...
{
    _host = adapter;
    LOG_GEKKO_INFO("[Gekko][GameSession] Net adapter set=%p", adapter);
}
...
i32 Gekko::GameSession::AddActor(GekkoPlayerType type, GekkoNetAddress* addr)
{
    ...
    if (type == GekkoSpectator) {
        ...
        LOG_GEKKO_INFO("[Gekko][GameSession] AddActor handle=%u type=%d locals=%zu remotes=%zu",
                       new_handle, (int)type, _msg.locals.size(), _msg.remotes.size());
...
{
    // Mirror UpdateSession's pre-start handshake path without generating
    // gameplay events. This lets a caller drive sync/connection progress
    // while the game itself is still frozen on a load barrier.
    _msg.session_events.Reset();
    Poll();

    if (!_started) {
        LOG_GEKKO_DEBUG("[Gekko][GameSession] NetworkPoll pre-start locals=%zu remotes=%zu",
                        _msg.locals.size(), _msg.remotes.size());
        AllActorsValid();
    }
}

i32 Gekko::GameSession::CurrentFrame()
{
    return _sync.GetCurrentFrame();
}

i32 Gekko::GameSession::LastReceivedFrame(i32 player)
{
    return _sync.GetLastReceivedFrom(player);
}

i32 Gekko::GameSession::MinReceivedFrame()
{
...
bool Gekko::GameSession::AllActorsValid()
{
	if (!_started) {
		if (!_msg.CheckStatusActors()) {
            LOG_GEKKO_DEBUG("[Gekko][GameSession] Waiting for actors to finish sync");
			return false;
		}

		// if none returned that the session is ready!
        _msg.session_events.AddSessionStartedEvent();
        LOG_GEKKO_INFO("[Gekko][GameSession] All actors valid — session started");
