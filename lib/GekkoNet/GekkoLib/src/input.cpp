...
void Gekko::InputBuffer::SetDelay(u8 delay)
{
	...
	if (_input_delay < delay) {
		u8 diff = delay - _input_delay;
		_input_delay = delay;

		Frame last_input = _last_received_input;
        u8* prev = _inputs[last_input % _buff_size]->input.get();

		// Only fill the difference in frames, not the full new delay.
		// Filling _input_delay frames would advance _last_received_input
		// too far, causing subsequent AddLocalInput calls to be silently
		// dropped by the strict sequential check in AddInput.
